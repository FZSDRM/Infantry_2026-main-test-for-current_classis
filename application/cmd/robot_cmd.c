...
// #include "can_comm.h" // 云台视觉专用分支不启用底盘 CAN 通信
// static CANCommInstance *cmd_can_comm; // 云台视觉专用分支不启用底盘 CAN 通信
#endif
...
// static Publisher_t *shoot_cmd_pub; // 云台视觉专用分支不发布发射控制消息
// static Subscriber_t *shoot_feed_sub; // 云台视觉专用分支不订阅发射反馈消息
static Shoot_Ctrl_Cmd_s shoot_cmd_send; // 保留为原切换逻辑使用的内部状态
// static Shoot_Upload_Data_s shoot_fetch_data; // 云台视觉专用分支不接收发射反馈
...
static float VisionYawSafeValue(float yaw, float fallback)
{
    if (!isfinite(yaw))
        return fallback;
    ...
    return loop_float_constrain(yaw, -180.0f, 180.0f);
}

static float VisionPitchSafeValue(float pitch, float fallback)
{
    if (!isfinite(pitch))
        return fallback;

    if (fabsf(pitch) > 180.0f)
        return fallback;

    return float_constrain(pitch, PITCH_MIN_LIMIT, PITCH_MAX_LIMIT);
}

static float VisionYawToNearestTurn(float yaw, float reference)
{
    while (yaw - reference > 180.0f)
        yaw -= 360.0f;
...
static void UpdateVisionInterpolation(void)
{
    ...
    if (!vision_interp_initialized)
    {
        ...
        target_yaw = VisionYawToNearestTurn(vision_recv_cache.aimYaw, gimbal_fetch_data.gimbal_imu_data.YawTotalAngle);
        vision_interp_start_yaw = gimbal_fetch_data.gimbal_imu_data.YawTotalAngle;
        vision_interp_target_yaw = target_yaw;
    ...
    target_yaw = VisionYawToNearestTurn(vision_recv_cache.aimYaw, current_yaw);

    target_changed =
        (fabsf(target_yaw - vision_interp_target_yaw) > VISION_INTERP_EPSILON_DEG) ||
...
static void RefreshVisionRecvCache(void)
{
    ...
    vision_recv_cache.aimYaw = VisionYawSafeValue(vision_recv_cache.aimYaw, gimbal_fetch_data.gimbal_imu_data.YawTotalAngle);
    vision_recv_cache.aimPitch = VisionPitchSafeValue(vision_recv_cache.aimPitch, gimbal_fetch_data.gimbal_imu_data.Pitch);
}
...
{   
    ...
    // int shoot_rate = 0;
    // if (chassis_fetch_data.rest_heat > 40)
    //     shoot_rate = 18;
    // if (chassis_fetch_data.rest_heat <= 40)
    //     shoot_rate = 1.4;
...
void RobotCMDInit()
{
    ...
    // shoot_cmd_pub = PubRegister("shoot_cmd", sizeof(Shoot_Ctrl_Cmd_s)); // 云台视觉专用分支不注册发射控制消息
    // shoot_feed_sub = SubRegister("shoot_feed", sizeof(Shoot_Upload_Data_s)); // 云台视觉专用分支不注册发射反馈消息

#ifdef GIMBAL_BOARD
    // 云台视觉专用分支不初始化底盘 CAN 通信。
    ...
    // };
    // cmd_can_comm = CANCommInit(&comm_conf); // 云台视觉专用分支不启用底盘 CAN 通信
#endif // GIMBAL_BOARD
...
static void SyncGimbalCmdToCurrentPose(void)
{
...
{
    static uint8_t last_remote_vision_active = 0;
    ...
    // 鼠标右键 + 视觉tracking=1 → 自瞄; 否则鼠标控制
    {
        ...
        if (image_rc_data[TEMP].mouse.press_r && vision_tracking)
        {
...
#if defined(VISION_USE_VCP) || defined(VISION_USE_UART)
            {
                ...
                GetVisionInterpolatedAim(&interp_yaw, &interp_pitch, &interp_pitch_valid);
                interp_yaw = VisionYawToNearestTurn(interp_yaw, gimbal_cmd_send.yaw);
                yaw_diff = interp_yaw - gimbal_cmd_send.yaw;
                if (yaw_diff > 180)
                    yaw_diff -= 360;
                if (yaw_diff < -180)
...
void RobotCMDTask()
{
    ...
    // 从其他应用获取回传数据
#ifdef GIMBAL_BOARD
    // chassis_fetch_data = *(Chassis_Upload_Data_s *)CANCommGet(cmd_can_comm); // 云台视觉专用分支不接收底盘 CAN 数据
#endif // GIMBAL_BOARD
    // SubGetMessage(shoot_feed_sub, &shoot_fetch_data); // 云台视觉专用分支不接收发射反馈
    ...
    if (ImageRemoteIsOnline())
    {
        ...
        else
        {
            ...
            // 拷贝鼠标键盘数据
            if (image_rc_data[TEMP].rc.switch_sw != IMAGE_SW_LEFT)
            {
                ...
                rc_data[TEMP].rc.switch_left =
                    image_remote_auto_aim_enabled ? RC_SW_MID : RC_SW_DOWN;
            }
    ...
    {
        gimbal_was_zero_force = 1;
        last_control_switch_left = rc_data[TEMP].rc.switch_left;
        last_control_switch_right = rc_data[TEMP].rc.switch_right;
        chassis_cmd_send.chassis_mode = CHASSIS_ZERO_FORCE;
        gimbal_cmd_send.gimbal_mode = GIMBAL_ZERO_FORCE;
        shoot_cmd_send.shoot_mode = SHOOT_OFF;
        shoot_cmd_send.friction_mode = FRICTION_OFF;
    ...
    {
        // ======== 统一控制路径 ========
        if (gimbal_was_zero_force)
        {
            gimbal_cmd_send.yaw = gimbal_fetch_data.gimbal_imu_data.YawTotalAngle;
            gimbal_cmd_send.pitch = gimbal_fetch_data.gimbal_imu_data.Pitch;
            gimbal_was_zero_force = 0;
        }
        if (rc_data[TEMP].rc.switch_left != last_control_switch_left ||
            rc_data[TEMP].rc.switch_right != last_control_switch_right)
        {
    ...
    // yaw_for_vision = loop_float_constrain(gimbal_fetch_data.gimbal_imu_data.YawTotalAngle, -180.0f, 180.0f);
    ...
    // 转换角度为整数(放大100倍)
    vision_send_data_sp.yaw = (int32_t)(yaw_for_vision * 100.0f);
...
#ifdef GIMBAL_BOARD
    // CANCommSend(cmd_can_comm, (void *)&chassis_cmd_send); // 云台视觉专用分支不发送底盘 CAN 命令
#endif // GIMBAL_BOARD
    // PubPushMessage(shoot_cmd_pub, (void *)&shoot_cmd_send); // 云台视觉专用分支不发布发射命令
