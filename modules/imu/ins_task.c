...
attitude_t *INS_Init(void)
{
    while (BMI088Init(&hspi1, 1) != BMI088_NO_ERROR)
        ;
    // 使用robot_def.h中的宏定义初始化IMU参数
#ifdef IMU_PARAM_SCALE_X
    IMU_Param.scale[X] = IMU_PARAM_SCALE_X;
    IMU_Param.scale[Y] = IMU_PARAM_SCALE_Y;
    IMU_Param.scale[Z] = IMU_PARAM_SCALE_Z;
